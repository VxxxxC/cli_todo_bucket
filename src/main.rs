...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match &input {
        input if input[0] == "get" => {
            if &input[1] == "todo" {get_todo_api(uri).await};
            if &input[1] == "reminder" { println!("you are getting reminder!") }
        },
        ...
        input if input[0] == "check" => {list_todo_api(uri).await},
        _ => {},
    }
